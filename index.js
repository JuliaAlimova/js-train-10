...
//Виведе: { '43': 'Hello', '56': 42, a: 1, c: 3.14, d: 'World' }
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  else return Error('об\'єкт не було "заморожено"');
...
function getObjectValuesSum(obj) {
  ...
  return arrOfValues.reduce((sum, numb) => {
    return sum + numb;
  });
  // Повертаємо суму
}
...
function convertArrayToObj(arr) {
  ...
  // let newArr = arr.flat();
  // console.log(newArr);
  // newArr.filter(([key, value]) => {
  //   if (newArr.indexOf(key) === newArr.lastIndexOf(key)) return [key, value];
  // });
